...
def build_itos(vocab):
    """Return a dict mapping each index 0..len(voc[ab)-1 to its character."""
    itos = {}
    for i in range(len(vocab)):
        itos[i] = vocab[i]
...
def initialize_w_random(vocab_size, rng):
    """(vocab_size, vocab_size) float64 matrix of N(0,1) samples drawn from rng."""
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    N = len(ids)
    one_hot_batch = np.zeros((len(ids), vocab_size))
    for i in range(N):
        one_hot_batch[i][ids[i]] = 1
...
def forward_logits_lookup(w, ids):
    ...
    # w of shape (V, V), ids of shape (B,) -> (B, V)
...
def train_neural_bigram_loop(w, data, block_size, batch_size, learning_rate, num_steps, log_every):
    ...
    for i in range(num_steps):
        out = run_one_training_step(w, x, y, learning_rate)
        w = out['w']
        loss = out['loss']
...
def sample_from_neural_bigram(w, start_id, num_tokens, itos):
    ...
    tokens = np.array([start_id])
    logits = logits_to_probs_rowwise(w)
    max_prob_indexes = np.argmax(logits, 1)

    for i in range(num_tokens):
        current_token = tokens[-1]
        next_token = max_prob_indexes[current_token]
        tokens = np.append(tokens, next_token)

    return decode_ids(tokens, itos)
